...
// Call API 
let data;
fetch('http://localhost:3000/list-Products')
    .then((res) => res.json())
    .then((res) => {
        data = res
        renderProduct()
...
function renderProduct() {
    ...
    <div class="product-info"  name="${item.id}">
        <div class="btn-add">
...
function updateQuantity(data) {
    for (let i = 0; i < arrayProduct.length; i++) {
        if (arrayProduct[i].id == data.id) {
            ...
            let quantityNew = arrayProduct[i].quantity + 1;
            console.log(quantityNew, 'NEwQuantity');
           let check = arrayProduct[i].total = arrayProduct[i].priceNew * quantityNew;
...
// * Click Button Increase And Decrease Quantity
    let selectQuantity = document.querySelectorAll('.select-quantity');
    selectQuantity.forEach((item, index) => {
        
    }) 
    const clickUpdateQuantity = () => {
    }
// * End
