...
const sum = () => {
    ...
    let payment = $('.info-total');
    let priceSale = 0;
    let totalpay = 0;
    let totalPrice = listProduct.reduce((acc, curr) => {
        acc += curr.priceOld == null ? curr.priceNew : curr.priceOld;
        priceSale += curr.priceOld == null ? 0 : (curr.priceNew - curr.priceOld);
        return acc;
    }, 0);
    subtotal.innerHTML = totalPrice.toLocaleString('en-US') + '₫';
    sale.innerHTML = `${priceSale.toLocaleString('en-US') + '₫'}`;
    payment.innerHTML = (totalPrice - sale).toLocaleString('en-US') + '₫'
};
sum()
